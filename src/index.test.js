...
test('test normalize json api pagination response', function () {
    var normalized = jsonApiNormalizer(paginationResponse);
...
test('test normalize json api pagination response only data', function () {
    var normalized = jsonApiNormalizer(paginationResponse, {
        onlyData: true
    });
...
test('test normalize json api pagination response events', function () {
    var counters = {
        resources: 0,
        dataResources: 0,
        relationships: 0
    ...
    jsonApiNormalizer(paginationResponse, {
        beforeRelationship: function (relationship) {
            counters.relationships++;
            return relationship;
        },
        beforeResource: function (resource) {
            counters.resources++;
            return resource;
        },
        beforeDataResource: function (resource) {
...
test('test normalize json api errors', function () {
    var normalized = jsonApiNormalizer(errorResponse);
    expect(normalized).toEqual({
...
test('test normalize json api error events', function () {
    var errorCounter = 0;
